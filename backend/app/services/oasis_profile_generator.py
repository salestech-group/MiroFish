...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social-media user-profile generation expert. Produce detailed, realistic personas for opinion simulation, faithfully reflecting the existing real-world situation. You must return a valid JSON object; string values must not contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social-media user persona for the entity, faithfully reflecting the existing real-world situation.
...
2. persona: detailed persona description (a single plain-text paragraph of around 2000 characters) that must cover:
   ...
   - Stance and views (attitude toward the topic, what might provoke or move them)
   - Distinctive traits (catchphrases, unique experiences, personal hobbies)
   - Personal memory (an essential part of the persona describing this individual's connection to the event, plus actions and reactions they have already taken in the event)
3. age: age as a number (must be an integer)
4. gender: gender, must be the English value "male" or "female"
...
Important:
- All field values must be strings or numbers, with no unescaped newline characters
- persona must be a single coherent paragraph of text
- {get_language_instruction()} (the gender field must use the English value "male" or "female")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social-media account profile for the institutional/group entity, faithfully reflecting the existing real-world situation.
...
1. bio: official-account bio, around 200 characters, professional and appropriate
2. persona: detailed account-profile description (a single plain-text paragraph of around 2000 characters) that must cover:
   - Institution basics (formal name, nature of the institution, founding background, primary functions)
   - Account positioning (account type, target audience, core function)
   - Voice and style (language characteristics, common expressions, taboo topics)
   ...
   - Special notes (the group profile this account represents, operational habits)
   - Institutional memory (an essential part of the persona describing this institution's connection to the event, plus actions and reactions it has already taken in the event)
...
- persona must be a single coherent paragraph of text with no unescaped newline characters
...
- The institutional account's voice must align with its identity"""
